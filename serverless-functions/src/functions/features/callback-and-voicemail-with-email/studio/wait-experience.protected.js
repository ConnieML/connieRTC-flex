...
exports.handler = async (context, event, callback) => {
  ...
  switch (mode) {
    ...
    case 'record-voicemail':
      ...
      // Build the callback URLs with voicemailOnly parameter if present
      ...
      if (isVoicemailOnly) {
        ...
        // FALLBACK: If recording fails or caller hangs up, say message and hang up
        twiml.say(options.sayOptions, options.messages.voicemailNotCaptured);
        twiml.hangup();
      } else {
        // Original working logic for regular callback/voicemail (7027079496)
    ...
    case 'voicemail-recorded':
          ...
          try {
            const taskAttributes = {
              ...
              callBackData: {
                ...
                transcription: event.TranscriptionText || 'Pending transcription...',
                phoneNumber: event.Called,
                attempts: 0
              }
            };

            const newVoicemailTask = await TaskRouterOperations.createTask({
              context,
            ...
            console.log(`Voicemail-only task created: ${newVoicemailTask.sid}`);
            ...
            console.error('Error creating voicemail-only task:', error);
            twiml.say(options.sayOptions, options.messages.processingError);
          }
        } else {
          console.log('Voicemail-only: No recording captured');
          twiml.say(options.sayOptions, options.messages.voicemailNotCaptured);
        }
      ...
      // Original voicemail-recorded logic for regular callback/voicemail (7027079496)
