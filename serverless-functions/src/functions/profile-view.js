exports.handler = async (context, event, callback) => {
  try {
    // Enhanced debug logging to see what parameters are received
    console.log('profile-view received event:', JSON.stringify(event));
    
    // Extract phone number and log it specifically
    const phone = event.phone || event.From || '';
    console.log(`Processing profile for phone: "${phone}" (from parameters: phone=${event.phone}, From=${event.From})`);
    
    // If no phone number is provided, show a "no profile" message instead of defaulting to a test user
    if (!phone) {
      console.log('No phone number provided, showing empty profile');
      return renderEmptyProfile(callback);
    }
    
    // Use the provided parameters without hardcoded defaults
    const profileId = event.profileId || `Unknown-${phone}`;
    const name = event.name || 'Unknown User';
    ...
    // Add a more permissive Content-Security-Policy that includes all possible Flex domains
    response.appendHeader('Content-Security-Policy', 
      "frame-ancestors 'self' https://flex.twilio.com https://*.flex.twilio.com https://*.twilio.com https://*.twil.io https://connie.team https://*.connie.team https://dev.connie.team https://portal.connie.team http://localhost:* https://*.augmentcxm.com");
...
// Helper function to render an empty profile when no phone number is provided
function renderEmptyProfile(callback) {
  const html = `
  <!DOCTYPE html>
  <html>
  <head>
  ...
  <body>
    <div class="card">
      <div class="message">
        <h1>No Profile Information</h1>
        <p>No phone number was provided to look up a profile.</p>
        <p>Please ensure a valid phone number is passed to this function.</p>
      </div>
    </div>
